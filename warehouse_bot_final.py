...
# Ozon Performance API (реклама) — отдельные ключи и хост, отдельная OAuth2-авторизация
...
class SupabaseService:
    ...
    # --- Reference data ---
    def get_suppliers_list(self, type_filter: str = None, include_inactive: bool = False):
        """Контрагенты. Если указан type_filter — только контрагенты этого типа. По умолчанию только
        активные (для выбора в новых операциях); include_inactive=True — для истории/баланса, где
        удалённые (is_active=False) контрагенты должны оставаться видимыми."""
    ...
    def deactivate_counterparty(self, cp_id: int):
        """Мягкое удаление — история операций (RESTRICT по FK) остаётся нетронутой."""
        self.client.table("counterparties").update({"is_active": False}).eq("id", cp_id).execute()
    ...
    def get_all_product_names(self):
        """Названия сырья/товаров по всем ИП сразу, без привязки — для прихода на склад. Только активные —
        удалённый товар не должен предлагаться при выборе для новой операции."""
        res = self.client.table("products").select("name").eq("is_active", True).order("name").execute()
        seen, names = set(), []
        for r in (res.data or []):
            n = r["name"]
            if n not in seen:
    ...
    def get_active_products(self) -> list:
        res = self.client.table("products").select("id,name").eq("is_active", True).order("name").execute()
        return res.data or []
    ...
    def get_purchases_report_rows(self, category_id: int, date_from: str, date_to: str) -> list:
        """Закупки (сырьё или расходники, по category_id) за период — для read-only отчётов роли
        'бухгалтер'. Те же поля, что пишет supply_confirm: item_name/quantity/price/amount/counterparty."""
        res = (
            self.client.table("operations")
            .select("operation_date,item_name,quantity,price,amount,counterparties(name)")
    ...
    def get_salary_report_rows(self, date_from: str, date_to: str) -> list:
        """Начисления зарплаты (category 'Зарплата') за период — read-only отчёт роли 'бухгалтер'."""
        cat_id = self.get_category_id("Зарплата")
        res = (
            self.client.table("operations")
            .select("operation_date,amount,comment,counterparties(name)")
    ...
    def get_other_warehouse_expenses_rows(self, exclude_category_ids: list, date_from: str, date_to: str) -> list:
        """Прочие расходы склада (весь group_name='склад' в categories, КРОМЕ закупки сырья/расходников —
        у них уже свои отдельные отчёты) за период — read-only отчёт роли 'бухгалтер'."""
        cat_ids = [
            c["id"] for c in (self.client.table("categories").select("id").eq("group_name", "склад").execute().data or [])
            if c["id"] not in exclude_category_ids
        ]
        if not cat_ids:
            return []
        res = (
            self.client.table("operations")
            .select("operation_date,amount,comment,categories(name),counterparties(name)")
    ...
    def get_supply_acceptances_missing_extra_fields(self) -> list:
        """Строки, у которых ещё не проставлен supply_completed_at или order_number (бэкафилл для записей,
        сохранённых до того, как эти поля стали синкаться). Триггер именно на эти два поля, а не на
        cluster: у части supply macrolocal_cluster_id реально отсутствует в Ozon (подтверждено данными)
        и останется NULL навсегда — если бы триггерили на cluster, эти строки перезапускали бы бэкафилл
        на каждом прогоне без результата, а строки с уже проставленным cluster, но без даты/номера заявки,
        никогда бы не попали на повторную попытку."""
        res = (
            self.client.table("ozon_supply_acceptances").select("supply_number,raw_json")
            .or_("supply_completed_at.is.null,order_number.is.null")
            .execute()
        )
        return res.data or []

    def update_supply_acceptance_extra_fields(self, supply_number: str, shipment_date, supply_completed_at, cluster, order_number):
        """Обновляет ВСЕ строки поставки разом — поля относятся к supply/заявке целиком, не к конкретному sku."""
        self.client.table("ozon_supply_acceptances").update({
            "shipment_date": shipment_date, "supply_completed_at": supply_completed_at,
    ...
    def get_hourly_employees(self):
        """Сотрудники с почасовой ставкой (для учёта смен). Только активные."""
        res = (
            self.client.table("counterparties").select("id,name,hourly_rate")
            .eq("type", "сотрудник").eq("is_active", True).not_.is_("hourly_rate", "null").order("name").execute()
        )
        return res.data or []

    def get_all_employees(self, include_inactive: bool = False):
        """Все сотрудники (для начисления фиксированного оклада). По умолчанию только активные."""
        q = self.client.table("counterparties").select("id,name,hourly_rate").eq("type", "сотрудник")
        if not include_inactive:
            q = q.eq("is_active", True)
        res = q.order("name").execute()
        return res.data or []
    ...
    def get_user_role(self, user_id: int) -> str:
        """'admin' — отдельная проверка через ADMIN_ID, не через колонку role (по инструкции — не трогать
        эту часть). Для остальных — role из approved_users, по умолчанию 'staff' (текущее поведение)."""
        if user_id == ADMIN_ID:
            return "admin"
        res = self.client.table("approved_users").select("role").eq("user_id", user_id).limit(1).execute()
        data = res.data or []
...
def get_main_menu_keyboard(user_id, role: str = "staff"):
    ...
    if role == "accountant":
        kb = [["🌰 Закупки сырья", "📦 Расходники"], ["💵 Зарплаты", "📉 Прочие расходы склада"]]
        return ReplyKeyboardMarkup(kb, resize_keyboard=True)

    kb = [["📦 Закупка", "💰 Оплата"], ["🏭 Склад", "💵 Продажа"], ["📜 История", "📊 Баланс"], ["➕ Добавить"]]
    return ReplyKeyboardMarkup(kb, resize_keyboard=True)
...
async def supply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get("db").get_user_role(update.effective_user.id) == "accountant":
...
async def balance_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t == "🔍 Один контрагент":
        sups = [s["name"] for s in db.get_suppliers_list(include_inactive=True)]
...
async def history_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = context.bot_data.get("db")
    sups = [s["name"] for s in db.get_suppliers_list(type_filter=TYPE_BUTTON_TO_DB[t], include_inactive=True)]
...
# --- Ozon Performance API (реклама) — отдельная OAuth2-авторизация ---
_ozon_performance_token_cache = {"token": None, "expires_at": None}


async def get_ozon_performance_token(client_id: str, client_secret: str) -> dict:
    """Получает/кэширует Bearer-токен Performance API через POST /api/client/token. Схема запроса/ответа
    ({"client_id","client_secret","grant_type":"client_credentials"} -> {"access_token","expires_in",
    "token_type"}, обычно expires_in=1800/30 мин) подтверждена по множеству независимых реальных
    интеграций на GitHub (не один источник) — но сама схема ни разу не проверялась живым вызовом на
    наших ключах, это первая проверка. Кэш обновляется за 2 минуты до истечения, чтобы не дёргать токен
    на каждый вызов."""
    now = datetime.now(TZ_MSK)
    cached = _ozon_performance_token_cache
    if cached["token"] and cached["expires_at"] and now < cached["expires_at"]:
        return {"access_token": cached["token"], "cached": True, "expires_at": cached["expires_at"]}

    async with httpx.AsyncClient(timeout=30.0) as http:
        resp = await http.post(
            f"{OZON_PERFORMANCE_API_BASE}/api/client/token",
    ...
    if not access_token or not expires_in:
        raise RuntimeError(f"Неожиданный ответ /api/client/token (нет access_token/expires_in): {data}")

    cached["token"] = access_token
    cached["expires_at"] = now + timedelta(seconds=expires_in - 120)
    return {"access_token": access_token, "cached": False, "expires_in": expires_in, "expires_at": cached["expires_at"]}


async def ozon_performance_token_debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННАЯ команда — проверяет получение токена Performance API. Не показывает сам токен, только
    подтверждение успеха и expires_in, как просили. Уберём вместе с остальной диагностикой этой задачи."""
    if update.effective_user.id != ADMIN_ID:
        return
    if not (OZON_PERFORMANCE_CLIENT_ID and OZON_PERFORMANCE_CLIENT_SECRET):
        ...
        return
    await update.message.reply_text("🔄 Проверяю получение токена Performance API...", reply_markup=get_main_menu_keyboard(update.effective_user.id))
    try:
        result = await get_ozon_performance_token(OZON_PERFORMANCE_CLIENT_ID, OZON_PERFORMANCE_CLIENT_SECRET)
        expires_at_str = result["expires_at"].strftime("%d.%m.%Y %H:%M:%S")
        if result.get("cached"):
            text = f"✅ Токен уже был в кэше (получен ранее в этом запуске бота).\nДействителен примерно до: {expires_at_str} МСК"
        else:
            text = f"✅ Токен Performance API получен успешно.\nexpires_in: {result['expires_in']} сек\nДействителен примерно до: {expires_at_str} МСК"
        await update.message.reply_text(text, reply_markup=get_main_menu_keyboard(update.effective_user.id))
    except Exception as e:
        logger.exception("DEBUG: не удалось получить токен Performance API")
...
async def request_ozon_performance_orders_report(client_id: str, client_secret: str, campaign_ids: list, date_from: str, date_to: str) -> str:
    """ВРЕМЕННО: только заказ отчёта — POST .../statistic/orders/generate -> UUID, без ожидания
    готовности (на реальных данных генерация явно дольше 60 сек, синхронно ждать больше не пытаемся —
    UUID проверяется отдельной командой, когда удобно)."""
    token_info = await get_ozon_performance_token(client_id, client_secret)
    ...
    if not uuid:
        raise RuntimeError(f"нет UUID в ответе generate: {gen_data}")
    return uuid


async def check_ozon_performance_orders_report_status(client_id: str, client_secret: str, uuid: str) -> tuple:
    """ВРЕМЕННО: ОДНА проверка готовности отчёта по UUID (без поллинга/ожидания) — 404 = ещё готовится,
    200 = готов (CSV или ZIP с CSV внутри). Возвращает (ready: bool, preview_or_None, error_or_None)."""
    token_info = await get_ozon_performance_token(client_id, client_secret)
    headers = {"Authorization": f"Bearer {token_info['access_token']}"}
    async with httpx.AsyncClient(timeout=30.0) as http:
    ...
    if status_resp.status_code >= 400:
        return False, None, f"status={status_resp.status_code}: {status_resp.text[:1500]}"
    ...
    if raw[:2] == b"PK":  # ZIP-сигнатура
        try:
            ...
            return True, f"[из ZIP, файл {name}]\n{preview}", None
        except Exception as e:
            return True, None, f"не смог распаковать ZIP-ответ: {e}"
    return True, raw[:2000].decode("utf-8", errors="replace"), None


async def ozon_performance_debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННАЯ команда — ВЕСЬ список кампаний целиком (не сэмпл), без отчёта. Ищем среди них кампанию
    с реальным бюджетом/названием (не REF_BLOGGER с пустым title/budget=0), чтобы дальше заказать отчёт
    именно по ней, а не по первым 5 подряд. Только диагностика, без сохранения."""
    if update.effective_user.id != ADMIN_ID:
        return
    if not (OZON_PERFORMANCE_CLIENT_ID and OZON_PERFORMANCE_CLIENT_SECRET):
        await update.message.reply_text("⚠️ OZON_PERFORMANCE_CLIENT_ID/OZON_PERFORMANCE_CLIENT_SECRET ещё не настроены в переменных окружения.")
    ...
    try:
        ...
        text = f"🐞 DEBUG — /api/client/campaign вернул {len(campaigns)} кампаний, все целиком:\n" + json.dumps(campaigns, ensure_ascii=False, indent=2, default=str)
        for chunk in _chunk_text_by_lines(text):
            await update.message.reply_text(chunk)
        await update.message.reply_text(
            "Дальше: выбери campaign_id нужной кампании (не REF_BLOGGER) и закажи по ней отчёт кнопкой «📢 Заказать отчёт».",
            reply_markup=get_main_menu_keyboard(update.effective_user.id),
        )
    except Exception as e:
...
async def perf_order_report_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННО: заказ отчёта по заказам без ожидания готовности — только запрос UUID. Статус проверяем
    отдельно, когда удобно (кнопка «📢 Статус отчёта»)."""
    if update.effective_user.id != ADMIN_ID:
        return ConversationHandler.END
    if not (OZON_PERFORMANCE_CLIENT_ID and OZON_PERFORMANCE_CLIENT_SECRET):
        await update.message.reply_text("⚠️ OZON_PERFORMANCE_CLIENT_ID/OZON_PERFORMANCE_CLIENT_SECRET ещё не настроены в переменных окружения.")
        return ConversationHandler.END
    await update.message.reply_text(
        "Введите campaign_id нужной кампании (можно несколько через запятую) — не REF_BLOGGER, а с реальным бюджетом/названием:",
        reply_markup=get_step_keyboard(),
    )
    return PERF_ORDER_REPORT_CAMPAIGNS


async def perf_order_report_campaigns(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t in ("❌ Главное меню", "🔙 Назад"): return await cancel_to_menu(update, context)
    campaign_ids = [c.strip() for c in t.split(",") if c.strip()]
    if not campaign_ids:
        await update.message.reply_text("⚠️ Не понял campaign_id. Введите ещё раз (можно несколько через запятую):", reply_markup=get_step_keyboard())
        return PERF_ORDER_REPORT_CAMPAIGNS

    t_now = datetime.now(TZ_MSK)
    date_from = (t_now.date() - timedelta(days=7)).isoformat()
    date_to = t_now.date().isoformat()
    await update.message.reply_text(f"🔄 Заказываю отчёт по заказам за {date_from} — {date_to} для {campaign_ids}...")
    try:
        uuid = await request_ozon_performance_orders_report(OZON_PERFORMANCE_CLIENT_ID, OZON_PERFORMANCE_CLIENT_SECRET, campaign_ids, date_from, date_to)
        await update.message.reply_text(
            f"✅ Отчёт заказан. UUID:\n`{uuid}`\n\nСохрани его — через несколько минут проверь готовность кнопкой «📢 Статус отчёта».",
            reply_markup=get_main_menu_keyboard(update.effective_user.id), parse_mode="Markdown",
        )
    except Exception as e:
        logger.exception("DEBUG: не удалось заказать отчёт по заказам Performance API")
        await update.message.reply_text(f"⚠️ Ошибка заказа отчёта: {e}", reply_markup=get_main_menu_keyboard(update.effective_user.id))
    return ConversationHandler.END


async def perf_check_report_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННО: одна проверка готовности отчёта по сохранённому UUID, без ожидания/поллинга."""
    if update.effective_user.id != ADMIN_ID:
        return ConversationHandler.END
    if not (OZON_PERFORMANCE_CLIENT_ID and OZON_PERFORMANCE_CLIENT_SECRET):
        await update.message.reply_text("⚠️ OZON_PERFORMANCE_CLIENT_ID/OZON_PERFORMANCE_CLIENT_SECRET ещё не настроены в переменных окружения.")
        return ConversationHandler.END
    await update.message.reply_text("Введите UUID отчёта:", reply_markup=get_step_keyboard())
    return PERF_CHECK_REPORT_UUID


async def perf_check_report_uuid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t in ("❌ Главное меню", "🔙 Назад"): return await cancel_to_menu(update, context)
    try:
        ready, preview, error = await check_ozon_performance_orders_report_status(OZON_PERFORMANCE_CLIENT_ID, OZON_PERFORMANCE_CLIENT_SECRET, t)
        if error:
            text = f"⚠️ UUID={t}: {error}"
        elif not ready:
            text = f"⏳ UUID={t}: отчёт ещё готовится, попробуйте позже."
        else:
            text = f"🐞 DEBUG — отчёт готов (UUID={t}), первые символы:\n{preview}"
    except Exception as e:
        logger.exception("DEBUG: не удалось проверить статус отчёта Performance API")
        text = f"⚠️ Ошибка проверки статуса: {e}"
    for chunk in _chunk_text_by_lines(text):
        await update.message.reply_text(chunk, reply_markup=get_main_menu_keyboard(update.effective_user.id))
    return ConversationHandler.END
...
async def collect_supply_acceptance_lines(client_id: str, api_key: str, date_from: str, date_to: str) -> tuple:
    ...
    supplies, это не баг, а факт устройства API. order_number (человекочитаемый номер заявки вида
    "118175977-1") — не перепроверялся отдельным SQL-срезом с моей стороны, в отличие от остальных
    полей; если формат окажется не таким, поправим по факту."""
    ...
    for order in orders:
        ...
        for supply in order.get("supplies") or []:
            ...
            completed_supplies.append((
                ...
                supply_completed_at, supply.get("macrolocal_cluster_id"), order_number,
            ))

    items_by_bundle = await fetch_ozon_supply_bundles(client_id, api_key, [b for _, b, _, _, _, _, _ in completed_supplies])

    lines = []
    for supply_id, bundle_id, order_id, shipment_date, supply_completed_at, cluster, order_number in completed_supplies:
        for item in items_by_bundle.get(bundle_id, []):
            ...
            lines.append({
                ...
                "shipment_date": shipment_date, "supply_completed_at": supply_completed_at, "cluster": cluster,
...
def process_supply_acceptance_line(db: "SupabaseService", line: dict) -> str:
    ...
    db.insert_supply_acceptance(
        ...
        shipment_date=line.get("shipment_date"), supply_completed_at=line.get("supply_completed_at"),
        cluster=line.get("cluster"), order_number=line.get("order_number"),
    )
    return "processed"
...
async def backfill_supply_acceptance_extra_fields(db: "SupabaseService", client_id: str, api_key: str) -> int:
    """Одноразовый бэкафилл shipment_date/supply_completed_at/cluster/order_number для строк, сохранённых
    до того, как эти поля стали синкаться. Идемпотентно (выбирает только строки с supply_completed_at
    или order_number IS NULL), поэтому безопасно вызывать на каждом прогоне — на пустой выборке просто
    ничего не делает.
    ...
    order_id_by_supply = {}
    for r in rows:
        order_id = (r.get("raw_json") or {}).get("order_id")
        if order_id:
            order_id_by_supply[r["supply_number"]] = order_id
    if not order_id_by_supply:
        return 0

    orders = await fetch_ozon_supply_order_details(client_id, api_key, list({v for v in order_id_by_supply.values()}))
    supply_lookup = {}
    for order in orders:
        shipment_date = order.get("created_date")
        supply_completed_at = order.get("state_updated_date")
        ...
        for supply in order.get("supplies") or []:
            ...
            if supply_id is None:
                continue
            supply_lookup[str(supply_id)] = (shipment_date, supply_completed_at, supply.get("macrolocal_cluster_id"), order_number)

    updated = 0
    for supply_number in order_id_by_supply:
        found = supply_lookup.get(supply_number)
        if not found:
            continue
        shipment_date, supply_completed_at, cluster, order_number = found
        db.update_supply_acceptance_extra_fields(supply_number, shipment_date, supply_completed_at, cluster, order_number)
        updated += 1
    return updated
...
def _chunk_text_by_lines(text: str, limit: int = 3500) -> list:
    """Бьёт текст на части по границам строк, никогда не оставляя незакрытый ```-блок в чанке (карточки
    заявок в отчёте по поставкам — многострочные code-block'и). Первая версия резала по сырому смещению
    символов — ломала пары *звёздочек*. Вторая версия уважала границы строк, но откладывала разрез до
    закрытия ``` БЕЗ ограничения по длине — если между двумя точками, где можно безопасно резать
    (снаружи ```), помещалось больше limit символов (крупная заявка с кучей поставок, или просто много
    заявок подряд без разрыва), чанк неограниченно рос и на длинных периодах превышал предел Telegram
    в 4096 символов ("Message is too long"). Эта версия по-прежнему предпочитает резать СНАРУЖИ ```, но
    если нужный чанк всё равно перерос limit, а мы внутри блока — принудительно закрывает текущий чанк
    своим ``` и открывает новый с ``` заново, так что оба сообщения остаются валидным Markdown сами по
    себе, а лимит длины соблюдается всегда, а не только пока "повезёт" с расположением заявок."""
    chunks = []
    current = ""
    in_fence = False
...
async def fetch_ozon_cash_flow_statement_raw(client_id: str, api_key: str, date_from: str, date_to: str) -> dict:
    """ВРЕМЕННО: /v1/finance/cash-flow-statement/list — метод подтверждён в официальном changelog Ozon
    (пользователем), но точная форма запроса/ответа на реальных данных ещё НЕ проверена. Схема запроса
    ({"date": {"from","to"}, "page", "page_size", "with_details"}) взята из исходников стороннего
    Go-клиента (github.com/diPhantxm/ozon-api-client, ozon/reports.go) — судя по нему, "result.cash_flows"
    выглядит как агрегированный отчёт за период (суммы заказов/возвратов/комиссий/услуг/доставки), а НЕ
    как список отдельных выплат со статусом/датами/номером документа, который нужен по задаче. Не гадаем
    дальше — возвращаем СЫРОЙ ответ целиком, разберёмся на реальных данных, прежде чем писать таблицу
    и sync-логику."""
    body = {
        "date": {"from": f"{date_from}T00:00:00.000Z", "to": f"{date_to}T23:59:59.000Z"},
        "page": 1, "page_size": 1000, "with_details": True,
    }
    return await _ozon_api_post(client_id, api_key, "/v1/finance/cash-flow-statement/list", body)


async def _run_ozon_payouts_debug_and_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, date_from: str, date_to: str):
    await update.message.reply_text(f"🔄 Проверяю /v1/finance/cash-flow-statement/list за {_format_date_ru(date_from)} — {_format_date_ru(date_to)}, подождите...", reply_markup=get_main_menu_keyboard(update.effective_user.id))
    try:
        data = await fetch_ozon_cash_flow_statement_raw(OZON_BULAT_CLIENT_ID, OZON_BULAT_API_KEY, date_from, date_to)
        text = "🐞 DEBUG — сырой ответ /v1/finance/cash-flow-statement/list:\n" + json.dumps(data, ensure_ascii=False, indent=2, default=str)
        for chunk in _chunk_text_by_lines(text):
            await update.message.reply_text(chunk, reply_markup=get_main_menu_keyboard(update.effective_user.id))
    except Exception as e:
        logger.exception("DEBUG: не удалось получить /v1/finance/cash-flow-statement/list")
        await update.message.reply_text(f"⚠️ Ошибка запроса: {e}", reply_markup=get_main_menu_keyboard(update.effective_user.id))


async def ozon_payouts_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННО: пока только DEBUG-проверка сырого ответа (только админ) — таблицы/sync ещё нет,
    ждём подтверждения реальной структуры полей."""
    if update.effective_user.id != ADMIN_ID:
        return ConversationHandler.END
    if not (OZON_BULAT_CLIENT_ID and OZON_BULAT_API_KEY):
        ...
        return ConversationHandler.END
    kb = [["Последние 3 дня"], ["Последние 30 дней"], ["Свой период (ДД.ММ-ДД.ММ)"], ["❌ Главное меню"]]
    await update.message.reply_text("💰 *Выплаты Ozon* (DEBUG, задача в разработке)\n\nЗа какой период?", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True), parse_mode="Markdown")
    return OZON_PAYOUTS_PERIOD


async def ozon_payouts_period(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t == "❌ Главное меню": return await cancel_to_menu(update, context)
    t_now = datetime.now(TZ_MSK)
    ...
    if t == "Последние 3 дня":
        date_from = (t_now.date() - timedelta(days=3)).isoformat()
        date_to = t_now.date().isoformat()
        await _run_ozon_payouts_debug_and_reply(update, context, date_from, date_to)
        return ConversationHandler.END
    ...
    if "Свой период" in t:
        await update.message.reply_text("Введите период в формате ДД.ММ-ДД.ММ (например 01.07-31.07):", reply_markup=get_step_keyboard())
        return OZON_PAYOUTS_PERIOD_CUSTOM
...
async def ozon_payouts_period_custom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t == "❌ Главное меню": return await cancel_to_menu(update, context)
    try:
        start_raw, end_raw = t.split("-")
...
# --- Ozon Остатки (задача: полная картина остатков без задвоения между категориями) ---
STOCK_DEBUG_SKUS = [4839976987, 4839932178]  # Боярышник, Шиповник — тестовые SKU для сверки структуры
STOCK_RECONCILE_DATE_FROM = "2026-08-02"  # период со скриншота приложения Ozon, для сверки сумм по статусам
STOCK_RECONCILE_DATE_TO = "2026-08-16"
STOCK_RECONCILE_REFERENCE = "Ожидают сборки 138, Готовы к отгрузке 117, Доставляются 1433, Доставлены 4064, Отменены 166 (итого 5918)"


def _truncate_lists(obj, max_items: int = 2):
    """Обрезает все списки в структуре до первых max_items элементов — компактный DEBUG-дамп без
    привязки к ещё неизвестным путям полей (нельзя фильтровать по SKU, пока не знаем, где он лежит)."""
    if isinstance(obj, list):
        return [_truncate_lists(x, max_items) for x in obj[:max_items]]
    if isinstance(obj, dict):
        return {k: _truncate_lists(v, max_items) for k, v in obj.items()}
    return obj


async def fetch_ozon_analytics_stocks_raw(client_id: str, api_key: str, skus: list) -> dict:
    """ВРЕМЕННО: /v1/analytics/stocks — метод подтверждён в справочнике Ozon for dev (пользователем).
    Схема запроса {"skus":[...]} и полей ответа (available/valid/waiting_docs/expiring/transit/
    transit_defect/stock_defect/excess/requested/return_from_customer/return_to_seller — отдельные
    именованные счётчики, не один статус на единицу) взята из community Go-клиента, живым вызовом
    не подтверждена."""
    return await _ozon_api_post(client_id, api_key, "/v1/analytics/stocks", {"skus": skus})


async def fetch_ozon_product_stocks_raw(client_id: str, api_key: str) -> dict:
    """ВРЕМЕННО: /v4/product/info/stocks — схема запроса {"filter": {"visibility": "ALL"}, "limit": 1000,
    "last_id": ""} и ответа (items[].stocks[] = {"type": "fbo"/"fbs", "present", "reserved"}) взята из
    community-клиента, живым вызовом не подтверждена."""
    return await _ozon_api_post(client_id, api_key, "/v4/product/info/stocks", {
        "filter": {"visibility": "ALL"}, "limit": 1000, "last_id": "",
    })


async def fetch_ozon_returns_raw(client_id: str, api_key: str) -> dict:
    """ВРЕМЕННО: /v1/returns/list — схема запроса {"filter": {}, "limit", "last_id"} и ответа
    (returns[].visual.status — дискретный статус на единицу возврата, returns[].product.{sku,quantity})
    взята из community-клиента, живым вызовом не подтверждена."""
    return await _ozon_api_post(client_id, api_key, "/v1/returns/list", {"filter": {}, "limit": 100, "last_id": 0})


async def fetch_ozon_fbo_postings_page(client_id: str, api_key: str, date_from: str, date_to: str, offset: int, limit: int = 1000) -> list:
    """ВРЕМЕННО: /v2/posting/fbo/list — схема запроса {"dir","filter":{"since","to"},"limit","offset",
    "translit","with"} и ответа (result[].status — дискретный статус на отправление, result[].products[]
    .{sku,quantity}) взята из community-клиента. Пагинация — по offset (не last_id), тоже неподтверждено.
    Ответ на разных клиентах оборачивался то в "result" как список, то как {"postings"/"items": [...]} —
    поддерживаем оба варианта, не гадая, какой из них реальный, пока не увидим сами."""
    body = {
        "dir": "ASC",
        "filter": {"since": f"{date_from}T00:00:00.000Z", "to": f"{date_to}T23:59:59.000Z"},
        "limit": limit, "offset": offset, "translit": True,
        "with": {"analytics_data": False, "financial_data": False},
    }
    data = await _ozon_api_post(client_id, api_key, "/v2/posting/fbo/list", body)
    result = data.get("result")
    if result is None:
        result = data
    if isinstance(result, dict):
        return result.get("postings") or result.get("items") or []
    return result or []


async def _reconcile_fbo_status_counts(client_id: str, api_key: str, date_from: str, date_to: str) -> tuple:
    """Полная пагинация /v2/posting/fbo/list за период по ВСЕМ SKU, агрегация по status: и количество
    отправлений, и сумма quantity по товарам внутри — для сверки с цифрами из приложения Ozon
    (STOCK_RECONCILE_REFERENCE). Возвращает (posting_counts, unit_sums, один сырой пример отправления)."""
    offset = 0
    limit = 1000
    posting_counts = {}
    unit_sums = {}
    sample = None
    while True:
        postings = await fetch_ozon_fbo_postings_page(client_id, api_key, date_from, date_to, offset, limit)
        if not postings:
            break
        if sample is None:
            sample = postings[0]
        for p in postings:
            status = p.get("status") or "неизвестно"
            posting_counts[status] = posting_counts.get(status, 0) + 1
            qty = sum((item.get("quantity") or 0) for item in (p.get("products") or []))
            unit_sums[status] = unit_sums.get(status, 0) + qty
        if len(postings) < limit:
            break
        offset += limit
        await asyncio.sleep(0.3)
    return posting_counts, unit_sums, sample


async def _run_ozon_stock_debug_and_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔄 Проверяю 4 метода Ozon по остаткам + сверяю статусы отправлений, подождите (может занять минуту)...", reply_markup=get_main_menu_keyboard(update.effective_user.id))

    checks = [
        ("/v1/analytics/stocks", lambda: fetch_ozon_analytics_stocks_raw(OZON_BULAT_CLIENT_ID, OZON_BULAT_API_KEY, STOCK_DEBUG_SKUS)),
        ("/v4/product/info/stocks", lambda: fetch_ozon_product_stocks_raw(OZON_BULAT_CLIENT_ID, OZON_BULAT_API_KEY)),
        ("/v1/returns/list", lambda: fetch_ozon_returns_raw(OZON_BULAT_CLIENT_ID, OZON_BULAT_API_KEY)),
    ]
    for name, call in checks:
        try:
            data = await call()
            text = f"🐞 DEBUG — {name}:\n" + json.dumps(_truncate_lists(data), ensure_ascii=False, indent=2, default=str)
        except Exception as e:
            logger.exception(f"DEBUG: не удалось получить {name}")
            text = f"⚠️ {name}: ошибка запроса: {e}"
        for chunk in _chunk_text_by_lines(text):
            await update.message.reply_text(chunk)
        await asyncio.sleep(0.4)

    try:
        posting_counts, unit_sums, sample = await _reconcile_fbo_status_counts(
            OZON_BULAT_CLIENT_ID, OZON_BULAT_API_KEY, STOCK_RECONCILE_DATE_FROM, STOCK_RECONCILE_DATE_TO,
        )
        lines = [
            f"🐞 DEBUG — /v2/posting/fbo/list, сверка статусов за {_format_date_ru(STOCK_RECONCILE_DATE_FROM)} — {_format_date_ru(STOCK_RECONCILE_DATE_TO)} (все SKU):",
            "\nПо количеству отправлений (posting):",
        ]
        for status, count in sorted(posting_counts.items(), key=lambda x: -x[1]):
            lines.append(f"  {status}: {count}")
        lines.append("\nПо сумме quantity внутри отправлений:")
        for status, qty in sorted(unit_sums.items(), key=lambda x: -x[1]):
            lines.append(f"  {status}: {qty}")
        lines.append(f"\nСправочно, твои цифры из приложения Ozon: {STOCK_RECONCILE_REFERENCE}")
        lines.append("\nПример одного сырого отправления:")
        lines.append(json.dumps(sample, ensure_ascii=False, indent=2, default=str))
        text = "\n".join(lines)
        for chunk in _chunk_text_by_lines(text):
            await update.message.reply_text(chunk)
    except Exception as e:
        logger.exception("DEBUG: не удалось сверить статусы /v2/posting/fbo/list")
        await update.message.reply_text(f"⚠️ Сверка статусов не удалась: {e}", reply_markup=get_main_menu_keyboard(update.effective_user.id))
        return

    await update.message.reply_text("✅ DEBUG по остаткам Ozon завершён.", reply_markup=get_main_menu_keyboard(update.effective_user.id))


async def ozon_stock_debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ВРЕМЕННАЯ команда — только диагностика, без сохранения. Уберём вместе с проверкой полей."""
    if update.effective_user.id != ADMIN_ID:
        return
    if not (OZON_BULAT_CLIENT_ID and OZON_BULAT_API_KEY):
        await update.message.reply_text("⚠️ Ключи Ozon ещё не настроены в переменных окружения.")
        return
    await _run_ozon_stock_debug_and_reply(update, context)


# --- Роль "бухгалтер": read-only отчёты (без сценариев внесения операций) ---
ACCOUNTANT_REPORT_TYPES = {
    "🌰 Закупки сырья": "raw",
    "📦 Расходники": "packaging",
    "💵 Зарплаты": "salary",
    "📉 Прочие расходы склада": "other",
}


def _cp_name(row: dict) -> str:
    cp = row.get("counterparties")
    return (cp or {}).get("name") or "—"


def build_purchases_report_text(title: str, rows: list) -> str:
    if not rows:
        return f"{title}\n\nЗа период ничего не найдено."
    lines = [f"{title}\n"]
    total = 0.0
    for r in rows:
        ...
        total += amount
        qty = r.get("quantity")
        qty_str = f"{float(qty):.0f}" if qty is not None else "—"
        price = r.get("price")
        price_str = f"{float(price):.2f} ₽/ед" if price is not None else "—"
        lines.append(f"▫️ {_format_date_ru(r.get('operation_date'))} | {r.get('item_name') or '—'} | {qty_str} | {price_str} | {amount:.2f} ₽ | {_cp_name(r)}")
    lines.append(f"\n💰 Итого: {round(total,2)} ₽")
    return "\n".join(lines)


def build_salary_report_text(rows: list) -> str:
    if not rows:
        return "💵 Зарплаты\n\nЗа период ничего не найдено."
    lines = ["💵 Зарплаты\n"]
    total = 0.0
    for r in rows:
        amount = float(r["amount"])
        total += amount
        lines.append(f"▫️ {_format_date_ru(r.get('operation_date'))} | {_cp_name(r)} | {amount:.2f} ₽" + (f" | {r['comment']}" if r.get("comment") else ""))
    lines.append(f"\n💰 Итого: {round(total,2)} ₽")
    return "\n".join(lines)


def build_other_expenses_report_text(rows: list) -> str:
    if not rows:
        return "📉 Прочие расходы склада\n\nЗа период ничего не найдено."
    lines = ["📉 Прочие расходы склада\n"]
    total = 0.0
    for r in rows:
        amount = float(r["amount"])
        total += amount
        cat = (r.get("categories") or {}).get("name") or "—"
        lines.append(f"▫️ {_format_date_ru(r.get('operation_date'))} | {cat} | {amount:.2f} ₽ | {_cp_name(r)}" + (f" | {r['comment']}" if r.get("comment") else ""))
    lines.append(f"\n💰 Итого: {round(total,2)} ₽")
    return "\n".join(lines)
...
async def accountant_report_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return ACC_REPORT_PERIOD


async def _run_accountant_report(update: Update, context: ContextTypes.DEFAULT_TYPE, date_from: str, date_to: str):
    db = context.bot_data.get("db")
    report_type = context.user_data.get("acc_report_type")
    raw_cat_id = db.get_category_id("Закупка сырья (орехи/сухофрукты)")
    packaging_cat_id = db.get_category_id("Расходники (упаковка, скотч, этикетки)")

    if report_type == "raw":
        text = build_purchases_report_text("🌰 Закупки сырья", db.get_purchases_report_rows(raw_cat_id, date_from, date_to))
    elif report_type == "packaging":
        text = build_purchases_report_text("📦 Расходники", db.get_purchases_report_rows(packaging_cat_id, date_from, date_to))
    elif report_type == "salary":
        text = build_salary_report_text(db.get_salary_report_rows(date_from, date_to))
    elif report_type == "other":
        text = build_other_expenses_report_text(db.get_other_warehouse_expenses_rows([raw_cat_id, packaging_cat_id], date_from, date_to))
    else:
        text = "⚠️ Не понял тип отчёта, начните заново."

    role = db.get_user_role(update.effective_user.id)
    for chunk in _chunk_text_by_lines(text):
        await update.message.reply_text(chunk, reply_markup=get_main_menu_keyboard(update.effective_user.id, role))


async def accountant_report_period(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t == "❌ Главное меню": return await cancel_to_menu(update, context)
    t_now = datetime.now(TZ_MSK)
    ...
    if t == "Последние 3 дня":
        date_from = (t_now.date() - timedelta(days=3)).isoformat()
        date_to = t_now.date().isoformat()
        await _run_accountant_report(update, context, date_from, date_to)
        return ConversationHandler.END
    ...
    if "Свой период" in t:
        await update.message.reply_text("Введите период в формате ДД.ММ-ДД.ММ (например 01.07-31.07):", reply_markup=get_step_keyboard())
        return ACC_REPORT_PERIOD_CUSTOM
...
async def accountant_report_period_custom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t == "❌ Главное меню": return await cancel_to_menu(update, context)
    try:
        start_raw, end_raw = t.split("-")
...
async def add_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get("db").get_user_role(update.effective_user.id) == "accountant":
        return await cancel_to_menu(update, context)
    kb = [["🏢 Поставщик", "📦 Товар"], ["🗑 Удалить"], ["❌ Главное меню"]]
    await update.message.reply_text("➕ Что вы хотите сделать?", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
    return ADD_SELECT
...
async def add_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t == "🏢 Поставщик":
        ...
        kb = [["🏢 Поставщик", "👤 Сотрудник"], ["📦 Товар"], ["❌ Главное меню"]]
        await update.message.reply_text("Кого/что удалить?", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        return DELETE_TYPE
    return await cancel_to_menu(update, context)


async def delete_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t in ("❌ Главное меню", "🔙 Назад"): return await cancel_to_menu(update, context)
    db = context.bot_data.get("db")
    ...
    if t == "🏢 Поставщик":
        kind = "counterparty"
        items = [c for c in db.get_suppliers_list() if c["type"] != "сотрудник"]
    elif t == "👤 Сотрудник":
        kind = "counterparty"
        items = db.get_all_employees()
    elif t == "📦 Товар":
        kind = "product"
        items = db.get_active_products()
    else:
        kb = [["🏢 Поставщик", "👤 Сотрудник"], ["📦 Товар"], ["❌ Главное меню"]]
        await update.message.reply_text("⚠️ Не понял выбор. Нажмите один из вариантов на клавиатуре:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        return DELETE_TYPE
    ...
    if not items:
        await update.message.reply_text("Список пуст — удалять нечего.", reply_markup=get_main_menu_keyboard(update.effective_user.id))
        return ConversationHandler.END

    context.user_data["del_kind"] = kind
    context.user_data["del_map"] = {i["name"]: i["id"] for i in items}
    await update.message.reply_text("Кого/что удалить?", reply_markup=build_grid_keyboard(list(context.user_data["del_map"].keys()), columns=2))
    return DELETE_ITEM
...
async def delete_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.text.strip()
    if t in ("❌ Главное меню", "🔙 Назад"): return await cancel_to_menu(update, context)
    del_map = context.user_data.get("del_map", {})
    if t not in del_map:
        await update.message.reply_text("Не найдено в списке. Выберите вариант с клавиатуры.")
        return DELETE_ITEM
    context.user_data["del_name"] = t
    context.user_data["del_id"] = del_map[t]
    kb = [["✅ Да, удалить", "❌ Отмена"]]
    await update.message.reply_text(
        f"Удалить «{t}»?\nЗапись пометится неактивной и пропадёт из списков выбора — вся история операций сохранится.",
        reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True),
    )
...
async def delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t != "✅ Да, удалить":
        return await cancel_to_menu(update, context)
    db = context.bot_data.get("db")
    kind = context.user_data.get("del_kind")
    item_id = context.user_data.get("del_id")
    name = context.user_data.get("del_name")
    if kind == "counterparty":
        db.deactivate_counterparty(item_id)
    else:
        db.deactivate_product(item_id)
    await update.message.reply_text(f"🗑 «{name}» удалён(а). История операций сохранена.", reply_markup=get_main_menu_keyboard(update.effective_user.id))
    return ConversationHandler.END
